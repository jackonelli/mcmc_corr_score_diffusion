...
class AnnealedMUHADiffReuseSampler:
    ...
    def leapfrog_step(self, x, v, i):
        step_size = self._step_sizes[i]
        return _leapfrog_step(x, v, lambda _x: self._gradient_function(_x, i), step_size, self._mass_diag_sqrt, self._num_leapfrog_steps)
    ...
    def sample(self, key: RandomKey, n_samples: int):
        ...
        def body_fn(i, inputs):
            ...
            def energy_steps(xs_, grads_):
                ...
                e = jnp.concatenate((e, jnp.sum(grads_[1] * (xs_[1] - xs_[0]), axis=1).reshape(-1, 1)), axis=1)
                energy = jnp.trapz(e)
                for i in range(1, len(xs_)-1):
                    e = jnp.sum(grads_[i] * (xs_[i+1] - xs_[i]), axis=1).reshape(-1, 1)
                    e = jnp.concatenate((e, jnp.sum(grads_[i+1] * (xs_[i+1] - xs_[i]), axis=1).reshape(-1, 1)), axis=1)
                    energy += jnp.trapz(e)
                return energy
    ...
    def sample(self, key: RandomKey, n_samples: int):
        ...
        def body_fn(i, inputs):
            ...
            def energy_s(ss):
                diff = x_hat - x
                x_ = x + ss[0] * diff
                energy = jnp.sum(self._energy_function(x=x_, t=dist_ind) * diff, axis=1).reshape(-1, 1)
