...
class AnnealedHMCScoreSampler(Sampler):
    ...
    def leapfrog_step(self, x, v, i, text_embeddings):
        ...
        return _leapfrog_step(x, v, lambda _x: self._gradient_function(_x, i, text_embeddings), step_size,
                              self._mass_diag_sqrt[i], self._num_leapfrog_steps)
    ...
    def sample_step(self, x, t, text_embeddings=None):
        ...
        for i in range(self._num_samples_per_step):
            ...
            v_prime = v * self._damping_coeff + np.sqrt(1. - self._damping_coeff ** 2) * eps * self._mass_diag_sqrt[t]
...
def _leapfrog_step(x_0: torch.tensor, v_0: torch.tensor, gradient_target: Callable, step_size: float,
                   mass_diag_sqrt: torch.tensor, num_steps: int):
