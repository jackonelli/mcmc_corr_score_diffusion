...
def main():
    ...
    mcmc_methods = ['uhmc'] # ['ula', 'uhmc'] # ['hmc'] # ['hmc', 'la']
    mcmc_steps = [2] # [6, 2] # [2] # [2, 6]
    if args.parallell:
        np.random.seed(args.job_id)
    factors_ = [round(5*np.random.rand(), 2)] # [round(10 * np.random.rand() + 0.5, 2), round(5*np.random.rand(), 2)]
    exponents_ = [round(0.1 * np.random.rand() + 1.45, 2)] # [round(0.1 * np.random.rand() + 0.95, 2), round(0.1 * np.random.rand() + 1.45, 2)]
