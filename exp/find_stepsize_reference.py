...
from src.samplers.mcmc import (
    ...
    AdaptiveStepSizeReferenceMCMCSamplerWrapper,
    AdaptiveStepSizeReferenceMCMCSamplerWrapperSmallBatchSize,
...
def main():
    args = parse_args()
    accept_rate_reference = pickle.load(open(args.accept_rate_referemce_path, 'rb'))
    set_seed(args.seed)
    ...
    if batch_size < num_samples:
        sampler = AdaptiveStepSizeReferenceMCMCSamplerWrapperSmallBatchSize(
            sampler=mcmc_sampler,
            accept_rate_reference=accept_rate_reference,
        ...
        sampler = AdaptiveStepSizeReferenceMCMCSamplerWrapper(
            sampler=mcmc_sampler,
            accept_rate_reference=accept_rate_reference,
            marginal=args.marginal,
            time_steps=time_steps,
    ...
    adaptive_step_sizes = best_step_size(accept_rate_reference, adaptive_step_sizes)
    lower_bound, upper_bound = accept_rate_reference
...
def best_step_size(accept_rate_reference, a_step_sizes):
    idxs = np.sort([k for k in a_step_sizes.keys() if isinstance(k, int)])[:-1]
    a_step_sizes['best'] = {'accept': [], 'step_sizes': []}
    for i in idxs:
        alpha_ref = accept_rate_reference[i]
        idxmin = np.argmin(np.abs(np.array(a_step_sizes[i]['accepts']) - alpha_ref))
